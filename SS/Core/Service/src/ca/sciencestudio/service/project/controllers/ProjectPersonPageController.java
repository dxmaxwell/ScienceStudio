...
public class ProjectPersonPageController extends AbstractModelController {
	...
	public String projectPersonPage(@PathVariable String projectPersonGid, ModelMap model) throws Exception {
		...
		Data<Person> personData = personAuthzDAO.get(user, projectPerson.getPersonGid());
		
		Data<Project> projectData = projectAuthzDAO.get(user, projectPerson.getProjectGid());
		
		Data<Authorities> authoritiesData = projectAuthzDAO.getAuthorities(user, projectPerson.getProjectGid());
