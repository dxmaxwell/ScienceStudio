 ...
 *
 *  Description:
 *     Command line interface for CLS Data Acqusition library (libacq.so >= 3.2, libacqOutputHandler.so >= 3.2, libEpicsConnect.so >= 1.5).
 *
 */
...
#define ACQUISITIONCL_VERSION	"2.0.2"

#define CMD_BUFFER_SIZE 		128
#define MACRO_BUFFER_SIZE	512

#define TEXT_OUTPUT_HANDLER_NAME	"Text"
#define FILE_OUTPUT_STREAM_NAME 		"File"

#define DEFAULT_FILE_STREAM_TEMPLATE 		"acqdata.%03d.dat"
#define DEFAULT_FILE_STREAM_DIRECTORY		"."
#define DEFAULT_FILE_STREAM_SEQ_NUM 		"0"

void printHelp(const char* cmd);

void showMessage(acqMaster_t *master, const char *fmt, ...);
void showStatus(acqMaster_t *master, acqState mode);
void showProgress(acqMaster_t * master, const char *title, double percent);
...
int main(int argc, char *argv[]) {
	...
	const char *optstring = "m:M:f:d:u:n:s:e:p:i";
	char optprint[] = {0, 0};
	int optargc;
	char **optargv;
	...
	acqBaseStream *fileStream;
	acqTextSpectrumOutput *textOutput;
	...
	// Initialize Output Handlers and Streams //
	fileStream = acqFileStream::build_acqFileStream();
	fileStream->setProperty(PROP_FILE_TEMPLATE, DEFAULT_FILE_STREAM_TEMPLATE);
	fileStream->setProperty(PROP_FILE_PATH, DEFAULT_FILE_STREAM_DIRECTORY);
	fileStream->setProperty(PROP_SEQUENCE_NUMBER, DEFAULT_FILE_STREAM_SEQ_NUM);

	textOutput = new acqTextSpectrumOutput();
	textOutput->setSpectrumFormat(textOutput->SF_TEXT);
	textOutput->setBaseStream(fileStream);

	// Initialize Acquisition Master //
	master = new_acqMaster();
	master->globalShutdown = 1;
	master->globalPause = 0;

	// Initialize Callbacks //
	acqSetStatusMessage(showMessage, master);
	acqSetShowProgress(showProgress , master);
	acqSetPutMode(showStatus, master);
	acqSetOnStop(onStop, master);

	acqRegisterOutputHandler(master, (void*)textOutput, &textOutput->handler);

	acq_file_load(cfgFileName, master);
	...
	while((opt = getopt(optargc, optargv, optstring)) >= 0) {
		...
		switch (opt) {
			case 'f':
				fileStream->setProperty(PROP_FILE_TEMPLATE, optarg);
				break;

			case 'd':
				fileStream->setProperty(PROP_FILE_PATH, optarg);
				break;

			case 'u':
				fileStream->setProperty(PROP_SEQUENCE_NUMBER, optarg);
				break;
...
void printHelp(const char* cmd) {
	fprintf(stderr, "USAGE: %s configFileName [-i] [-m macro] [-M macroFile] [-f fileName] [-d dirName] [-n scan] [-s start] [-e end] [-p step]\n", cmd);
	fprintf(stderr, "\n\tVersion: %s\n", ACQUISITIONCL_VERSION);
	fprintf(stderr, "\n\tArguments:\n");
	...
	fprintf(stderr, "\t\t-u seqNum\tSpecify the initial sequence number for file name.\n");
...
void showProgress(acqMaster_t * master, const char *title, double percent) {
	showMessage(master, "Progress: %g\n", percent);	
}

void onStop(acqMaster_t *master) {
	showMessage(master, "Stopped!\n");
}
